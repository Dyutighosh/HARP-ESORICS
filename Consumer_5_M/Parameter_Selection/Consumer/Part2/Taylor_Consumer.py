...
def SubMainFunction(Scale, Intercept, filename):
    ...
    data = np.load(Path("ParameterSelection") / "Consumer" / "generated_sets" / "set2-1.npy", allow_pickle=False)
    ...
    data = np.load(Path("ParameterSelection") / "Consumer" / "generated_sets" / "set2-2.npy", allow_pickle=False)
    ...
    data = np.load(Path("ParameterSelection") / "Consumer" / "generated_sets" / "set2-3.npy", allow_pickle=False)
