...
def SubMainFunction(Scale, Intercept, filename):
    ...
    data = np.load(Path("ParameterSelection") / "Consumer" / "generated_sets" / "set3-1.npy", allow_pickle=False)
    ...
    data = np.load(Path("ParameterSelection") / "Consumer" / "generated_sets" / "set3-2.npy", allow_pickle=False)
    ...
    data = np.load(Path("ParameterSelection") / "Consumer" / "generated_sets" / "set3-3.npy", allow_pickle=False)
