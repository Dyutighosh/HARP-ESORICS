...
def SubMainFunction(Scale, Intercept, filename):
    ...
    data = np.load(Path("ParameterSelection") / "Consumer" / "generated_sets" / "set4-1.npy", allow_pickle=False)
    ...
    data = np.load(Path("ParameterSelection") / "Consumer" / "generated_sets" / "set4-2.npy", allow_pickle=False)
    ...
    data = np.load(Path("ParameterSelection") / "Consumer" / "generated_sets" / "set4-3.npy", allow_pickle=False)
