...
def SubMainFunction(Scale, Intercept, filename):
    ...
    data = np.load(Path("ParameterSelection") / "Supplier" / "generated_sets" / "set1-1.npy", allow_pickle=False)
    ...
    data = np.load(Path("ParameterSelection") / "Supplier" / "generated_sets" / "set1-2.npy", allow_pickle=False)
    ...
    data = np.load(Path("ParameterSelection") / "Supplier" / "generated_sets" / "set1-3.npy", allow_pickle=False)
