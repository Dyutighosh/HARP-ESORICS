...
def get_consumer_inverse_config_5m(counter: int) -> Optional[InverseConfig]:
    if counter == 1:
        return None
    if counter < 11:
        min_val = 105000000.0 * 0.95
        max_val = 105000753.0 / 0.95
        return InverseConfig(
            min_val=min_val,
            max_val=max_val,
            scale=0.05,
            ...
            k=50,
            terms=12,
            coeffs=tuple([-2.5021503699567322, 0.927674109467529, 1.7187621077868473, -1.1468652106209127]),
            correction_k=2,
        )
    if counter < 148:
        min_val = 105000830.0 * 0.95
        max_val = 105006501.0 / 0.95
        return InverseConfig(
            min_val=min_val,
            max_val=max_val,
            scale=0.4,
            intercept=0.8 - (0.4 / 2.0),
            k=14,
            terms=17,
            coeffs=tuple([-2.8712681630965813, 0.8989467137075691, 2.2688903017073825, 1.0737000231873117, -1.7903256674950274, -3.3847307466861993, 2.833223588847389]),
            correction_k=30,
        )
    if counter < 585:
        min_val = 105006528.0 * 0.95
        max_val = 105020854.0 / 0.95
        return InverseConfig(
            min_val=min_val,
            max_val=max_val,
            scale=0.2,
            ...
            k=50,
            terms=12,
            coeffs=tuple([-2.97214051672089, 1.6566051645087856, 1.8454262941528186, -0.5145807868341129, -2.2921355077520587, 1.2773949928368895]),
            correction_k=35,
        )
    min_val = 105020882.0 * 0.95
    max_val = 105032242.0 / 0.95
    return InverseConfig(
        min_val=min_val,
        max_val=max_val,
        scale=0.2,
        ...
        k=50,
        terms=12,
        coeffs=tuple([-2.547340219541774, 0.7099005930376031, 1.5817293268730304, 0.6608258051071602, -0.9822995225946655, -1.641594679717319, 1.2197172915272438]),
        correction_k=40,
    )


def get_supplier_inverse_config_5m(counter: int) -> Optional[InverseConfig]:
    if counter == 1:
        return None
    if counter < 11:
        min_val = 210.0 * 0.95
    ...
    if counter < 148:
        ...
        return InverseConfig(
            ...
            k=14,
            terms=18,
            coeffs=tuple([-18.776681441948337, 165.5530764674268, -906.6135733343324, 3427.322331587569, -9355.209685064447, 18572.58969581349, -25850.238896450566, 21787.249657738575, -3156.116001744685, -16320.226056958008, 15675.62932866588, 5130.338928119905, -19061.994771558773, 7005.275021150275, 15729.7532782759, -23755.67435251347, 15254.86560208723, -5017.3884582374885, 692.6617154629043]),
            correction_k=5,
        )
    if counter < 585:
        min_val = 6738.0 * 0.95
        max_val = 21064.0 / 0.95
        return InverseConfig(
            min_val=min_val,
            max_val=max_val,
            scale=0.35,
            intercept=0.8 - (0.35 / 2.0),
            k=14,
            terms=18,
            coeffs=tuple([-9.73298466985114, 38.301957704156614, -72.73732513390502, 51.29893149870725, 34.32431504007919, -54.35420759137169, -35.528932017808394, 46.541472597593874, 51.867405019507004, -26.947702836823453, -67.7452791408061, -3.2179460358959027, 73.95569039002493, 33.40638606587887, -78.09499731373243, -46.92651353313704, 118.90325618113222, -67.24013393500648, 12.926650777050378]),
            correction_k=4,
        )
    min_val = 21092.0 * 0.95
    max_val = 32452.0 / 0.95
    return InverseConfig(
        min_val=min_val,
        max_val=max_val,
        scale=0.1,
        ...
        k=30,
        terms=14,
        coeffs=tuple([-4.945680842325948, 7.594013984368148, -0.994900502784341, -5.13394680805275, -1.4509164824406386, 3.673778907959041, 4.106597662113166, -0.5200486627458701, -4.8780013647302, -3.092056243873703, 3.979364059476394, 5.632915606879711, -6.954135415513447, 1.9830406810570893]),
        correction_k=4,
    )
...
def update_consumer_prices_weighted(
    prices: Sequence[float],
    demands: Sequence[float],
    ...
    total_price = float(sum(float(price) * int(mult) for price, mult in zip(prices, multiplicities)))
    constant = 2.0 * eta / (s_dot_lambda_zero - w_dot_lambda_zero)
    ratio = total_supply / total_price
    value_term = 1.0 - (constant * ratio)
...
class HARPProtocol:
    def __init__(self, he: Pyfhel, slot_count: int, rotation: RotationConvention, rng: random.Random, consumer_inverse_profile: str = "1405", supplier_inverse_profile: str = "1405"):
        self.he = he
        self.slot_count = slot_count
        self.rotation = rotation
        self.rng = rng
        self.inverse = InverseApproximator(he)
        self.consumer_inverse_profile = consumer_inverse_profile
        self.supplier_inverse_profile = supplier_inverse_profile
    ...
    def _coeffwise_scale(self, coeffs: Sequence[int], scale: int, layout: CoeffLayout) -> List[int]:
        size = max(len(coeffs), layout.vector_size)
        out: List[int] = [0] * size
        active_limit = layout.active_modulus_count * layout.coeff_count
        scale_i = int(scale)
        for i in range(layout.active_modulus_count):
            modulus = layout.qi[i]
            offset = i * layout.coeff_count
            scale_mod = scale_i % modulus
            for j in range(layout.coeff_count):
                idx = offset + j
                coeff_v = int(coeffs[idx]) if idx < len(coeffs) else 0
                out[idx] = (coeff_v * scale_mod) % modulus
        for idx in range(active_limit, size):
            coeff_v = int(coeffs[idx]) if idx < len(coeffs) else 0
            out[idx] = coeff_v * scale_i
        return out
    ...
    def compute_group_ratios(
        ...
        consumer_multiplicities: Optional[Sequence[int]] = None,
        supplier_multiplicities: Optional[Sequence[int]] = None,
    ) -> ProtocolDebugInfo:
        ...
        if consumer_multiplicities is None:
            consumer_multiplicities = [1] * len(consumer_prices)
        if supplier_multiplicities is None:
            supplier_multiplicities = [1] * len(supplier_prices)

        clear_lambda_c = float(sum(float(price) * int(mult) for price, mult in zip(consumer_prices, consumer_multiplicities)))
        clear_supply = float(sum(float(supply) * int(mult) for supply, mult in zip(supplier_supplies, supplier_multiplicities)))
        clear_lambda_s = float(sum(float(price) * int(mult) for price, mult in zip(supplier_prices, supplier_multiplicities)))
        clear_demand = float(sum(float(demand) * int(mult) for demand, mult in zip(consumer_demands, consumer_multiplicities)))

        t0 = time.time()
        aggregate_share1 = [0] * self._top_layout.vector_size
        aggregate_share2 = [0] * self._top_layout.vector_size

        for idx, ((price, demand), multiplicity) in enumerate(zip(zip(consumer_prices, consumer_demands), consumer_multiplicities)):
            ...
            scale_mult = int(multiplicity)
            share1_scaled = share1 if scale_mult == 1 else self._coeffwise_scale(share1, scale_mult, layout)
            share2_scaled = share2 if scale_mult == 1 else self._coeffwise_scale(share2, scale_mult, layout)
            agg_dcu_start = time.time()
            aggregate_share1 = self._coeffwise_add(aggregate_share1, share1_scaled, layout)
            timings.dcu_aggregate_s += time.time() - agg_dcu_start
            agg_iso_start = time.time()
            aggregate_share2 = self._coeffwise_add(aggregate_share2, share2_scaled, layout)
        ...
        for idx, ((price, supply), multiplicity) in enumerate(zip(zip(supplier_prices, supplier_supplies), supplier_multiplicities)):
            sample_start = time.time()
        ...
        t2 = time.time()
        c_cfg = get_consumer_inverse_config_5m(counter) if self.consumer_inverse_profile == "5m" else get_consumer_inverse_config(counter)
        if c_cfg is None:
        ...
        s_cfg = get_supplier_inverse_config_5m(counter) if self.supplier_inverse_profile == "5m" else get_supplier_inverse_config(counter)
        if s_cfg is None:
...
class HARPSimulation:
    def __init__(
        ...
        debug_first_n: int,
        log_file: Path,
        target_consumers: Optional[int] = None,
        scaling_mode: str = "pattern-collapse",
    ):
        ...
        self.source_num_consumers = self.src.num_consumers
        self.num_consumers = int(target_consumers) if target_consumers is not None else self.src.num_consumers
        ...
        self.consumer_pattern_len = len(self.d_pattern)
        if self.scaling_mode == "pattern-collapse":
            base = self.num_consumers // self.consumer_pattern_len
            rem = self.num_consumers % self.consumer_pattern_len
            self.consumer_multiplicities = [base + (1 if i < rem else 0) for i in range(self.consumer_pattern_len)]
            self.consumer_state_pattern_indices = list(range(self.consumer_pattern_len))
        else:
            self.consumer_multiplicities = [1] * self.num_consumers
            self.consumer_state_pattern_indices = [i % self.consumer_pattern_len for i in range(self.num_consumers)]
        self.consumer_state_size = len(self.consumer_state_pattern_indices)

        inverse_profile = "5m" if self.num_consumers >= 5_000_000 else "1405"
        self.protocol = HARPProtocol(
            self.he, self.slot_count, self.rotation, self.rng,
            consumer_inverse_profile=inverse_profile,
            supplier_inverse_profile=inverse_profile,
        )
        ...
        total_w_dot = 0.0
        for mult, pattern_idx in zip(self.consumer_multiplicities, self.consumer_state_pattern_indices):
            d_i = self.d_pattern[pattern_idx]
            eph_i = self.eph_pattern[pattern_idx]
            total_w_dot += int(mult) * d_i * eph_i * (self.lambda0 ** (eph_i - 1.0))
    ...
    def _log_run_header(self) -> None:
        ...
        for i, q in enumerate(layout.qi):
            self._log(f'    q[{i}] = {q}')
        self._log('')
        if hasattr(self, 'scaling_mode') and self.scaling_mode == 'pattern-collapse':
            self._log('Population scaling shortcut: exact repeated-pattern collapse is active.')
            self._log(f'  Actual consumers simulated logically: {self.num_consumers}')
            self._log(f'  Representative consumer archetypes carried in memory: {self.consumer_state_size}')
            self._log(f'  Consumer type multiplicities: {self.consumer_multiplicities}')
            self._log('  Aggregate share sums are scaled by these multiplicities in the encoded coefficient domain.')
            self._log('')
    ...
    def _log_step_details(
        ...
        for item in inter.get('consumer_share_examples', []):
            ...
            self._log(f'    Consumer {idx + 1} (multiplicity {mult}):')
        ...
        for item in inter.get('supplier_share_examples', []):
            ...
            s2 = item.get('share2_slots_first4', [float('nan')] * 4)
            mult = item.get('multiplicity', 1)
            self._log(f'    Supplier {idx + 1} (multiplicity {mult}):')
    ...
    def _consumer_demands(self, prices: Sequence[float]) -> List[float]:
        out: List[float] = []
        for i, pattern_idx in enumerate(self.consumer_state_pattern_indices):
            d_i = self.d_pattern[pattern_idx]
            eph_i = self.eph_pattern[pattern_idx]
    ...
    @staticmethod
    def _weighted_sum(values: Sequence[float], multiplicities: Sequence[int]) -> float:
        return float(sum(float(v) * int(m) for v, m in zip(values, multiplicities)))

    def run(self) -> SimulationResult:
        plain_consumer_prices = [self.lambda0] * self.consumer_state_size
        priv_consumer_prices = [self.lambda0] * self.consumer_state_size
        ...
        for step in range(1, self.periods + 1):
            ...
            plain_total_demand = self._weighted_sum(plain_demands, self.consumer_multiplicities)
            plain_total_supply = float(sum(plain_supplies))
            priv_total_demand = self._weighted_sum(priv_demands, self.consumer_multiplicities)
            ...
            next_plain_consumer_prices, _, plain_consumer_ratio = update_consumer_prices_weighted(
                plain_consumer_prices,
                plain_demands,
                self.consumer_multiplicities,
                plain_total_supply,
            ...
            dbg = self.protocol.compute_group_ratios(
                ...
                consumer_multiplicities=self.consumer_multiplicities,
                supplier_multiplicities=[1] * self.num_suppliers,
            )
            next_priv_consumer_prices, _, _ = update_consumer_prices_weighted(
            ...
            next_priv_consumer_prices = [
                ...
                for i in range(self.consumer_state_size)
            ]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=5000000,
        help="Actual logical consumer population size (default: 5,000,000).",
    )
    parser.add_argument(
        "--scaling-mode",
        type=str,
        choices=["pattern-collapse", "none"],
        default="pattern-collapse",
        help="Use exact repeated-pattern collapse for large populations, or disable it.",
    )
...
def main() -> None:
    ...
    sim = HARPSimulation(
        ...
        target_consumers=args.target_consumers,
