...
# Add text to highlight standard deviation
mean_std = np.mean(std_data)
# plt.text(len(mean_data) // 2, np.mean(mean_data), f'Standard Deviation = {mean_std:.2f}', color='blue', fontsize=12, ha='center', va='center')

# Create proxy artists for the legend
proxy_artists = [plt.Line2D([0], [0], color='blue')] + [plt.Line2D([0], [0], color='none')] * len(legend_entries)

# Split the annotations into multiple legend boxes
mid_index = len(legend_entries) // 2

# Create the first legend box
first_legend = plt.legend(proxy_artists[:mid_index + 1], ['Mean'] + legend_entries[:mid_index], loc='upper left', fontsize='small')

# # Create the second legend box
second_legend = plt.legend(proxy_artists[mid_index:], [''] + legend_entries[mid_index:], loc='lower right', fontsize='small')

# Add the first legend box to the plot
plt.gca().add_artist(first_legend)

plt.xlabel('Sampling Windows', fontsize=12)
plt.ylabel('Total Pricing Signal', fontsize=12)
# plt.title('Mean and Standard Deviation of Data from .npy Files')
plt.show()
