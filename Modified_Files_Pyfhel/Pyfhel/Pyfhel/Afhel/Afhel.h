...
class Afhel {
 public:
  ...
  virtual std::vector<std::uint64_t> get_plaintext_qi(AfPtxt &ptxt) = 0;
  virtual std::vector<std::uint64_t> get_ciphertext_qi(AfCtxt &ctxt) = 0;
  virtual std::vector<std::uint64_t> plaintext_to_raw(AfPtxt &ptxt) = 0;
  virtual void plaintext_from_raw(std::vector<std::uint64_t> &raw, AfPtxt &ptxt) = 0;
