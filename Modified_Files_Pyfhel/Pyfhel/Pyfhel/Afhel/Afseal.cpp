                        ...
                        std::vector<uint64_t> qi_values)
{
  ...
  this->qi.clear();
  for (auto &modulus: parms.coeff_modulus())
  {
...
void Afseal::KeyGen()
{
  ...
  this->keyGenObj = make_shared<KeyGenerator>(afhel_context);
...
std::vector<uint64_t> Afseal::get_plaintext_qi(AfPtxt &ptxt)
{
  auto &plain = _dyn_p(ptxt);
  auto context_data = this->get_context()->get_context_data(plain.parms_id());
  if (!context_data)
  {
    throw std::logic_error("<Afseal>: Plaintext parms_id not found in context");
  }
  std::vector<uint64_t> qi_vals;
  for (auto &modulus : context_data->parms().coeff_modulus())
  {
    qi_vals.emplace_back(modulus.value());
  }
  return qi_vals;
}

std::vector<uint64_t> Afseal::get_ciphertext_qi(AfCtxt &ctxt)
{
  auto &cipher = _dyn_c(ctxt);
  auto context_data = this->get_context()->get_context_data(cipher.parms_id());
  if (!context_data)
  {
    throw std::logic_error("<Afseal>: Ciphertext parms_id not found in context");
  }
  std::vector<uint64_t> qi_vals;
  for (auto &modulus : context_data->parms().coeff_modulus())
  {
...
std::vector<uint64_t> Afseal::plaintext_to_raw(AfPtxt &ptxt)
{
  auto &plain = _dyn_p(ptxt);
  if (!plain.is_ntt_form())
  {
    throw runtime_error("<Afseal>: plaintext_to_raw expects an NTT plaintext");
  }
  auto context_data = this->get_context()->get_context_data(plain.parms_id());
  if (!context_data)
  {
  ...
  size_t coeff_modulus_count = context_data->parms().coeff_modulus().size();
  std::vector<uint64_t> raw(coeff_count * coeff_modulus_count);
...
void Afseal::plaintext_from_raw(std::vector<uint64_t> &raw, AfPtxt &ptxt)
{
  auto &plain = _dyn_p(ptxt);
  if (!plain.is_ntt_form())
  {
    throw runtime_error("<Afseal>: plaintext_from_raw expects an NTT plaintext");
  }
  auto context_data = this->get_context()->get_context_data(plain.parms_id());
  if (!context_data)
  {
  ...
  size_t coeff_count = context_data->parms().poly_modulus_degree();
  size_t coeff_modulus_count = context_data->parms().coeff_modulus().size();
  size_t expected = coeff_count * coeff_modulus_count;
  if (raw.size() != expected)
  {
    throw invalid_argument("<Afseal>: plaintext_from_raw size mismatch");
  }




  if (plain.dyn_array().size() != expected)
{
    throw invalid_argument("<Afseal>: plaintext_from_raw backing size mismatch");
}
...
#pragma omp parallel for
    ...
    {
      util::inverse_ntt_negacyclic_harvey(coeff_repr.begin() + (j * coeff_count), small_ntt_tables[j]);
    }
